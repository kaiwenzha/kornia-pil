...
def random_crop_size_and_crop_generator(
    batch_size: int,
    size: Tuple[int, int],
    scale: torch.Tensor,
    ratio: torch.Tensor,
    ...
    y_start, x_start, height, width = torch.chunk(params, 4, dim=1)

    crop_src = bbox_generator(
        x_start.view(-1).to(device=_device, dtype=_dtype),
        y_start.view(-1).to(device=_device, dtype=_dtype),
        width.view(-1).to(device=_device, dtype=_dtype),
        height.view(-1).to(device=_device, dtype=_dtype)
    )
    ...
    _input_size = torch.tensor(size, device=_device, dtype=torch.long).expand(batch_size, -1)
