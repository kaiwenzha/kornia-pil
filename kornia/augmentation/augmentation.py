...
class ColorJitter(IntensityAugmentationBase2D):
    ...
    def generate_parameters(self, batch_shape: torch.Size) -> Dict[str, torch.Tensor]:
        brightness: torch.Tensor = _range_bound(
            self.brightness, 'brightness', center=1.0, bounds=(0, float('inf')), device=self._device, dtype=self._dtype
        )
        contrast: torch.Tensor = _range_bound(
            self.contrast, 'contrast', center=1.0, bounds=(0, float('inf')), device=self._device, dtype=self._dtype
        )
        saturation: torch.Tensor = _range_bound(
            self.saturation, 'saturation', center=1.0, bounds=(0, float('inf')), device=self._device, dtype=self._dtype
        )
    ...
    def apply_transform(
        ...
        for idx in params['order'].tolist():
            t = transforms[idx]
            # print(f'jittered {idx}', jittered.min(), jittered.max())
            # jittered = torch.clamp(jittered, min=0., max=1.)
            jittered = t(jittered)
...
class RandomResizedCrop(GeometricAugmentationBase2D):
    ...
    def generate_parameters(self, batch_shape: torch.Size) -> Dict[str, torch.Tensor]:
        scale = torch.as_tensor(self.scale, device=self._device, dtype=self._dtype)
        ratio = torch.as_tensor(self.ratio, device=self._device, dtype=self._dtype)
        return rg.random_crop_size_and_crop_generator(
            batch_shape[0],
            (batch_shape[-2], batch_shape[-1]),
            scale,
            ratio,
