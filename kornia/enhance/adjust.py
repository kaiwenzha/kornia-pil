...
def adjust_saturation(input: torch.Tensor, saturation_factor: Union[float, torch.Tensor]) -> torch.Tensor:
    ...
    x_adjust: torch.Tensor = (1 - saturation_factor) * rgb_to_grayscale(input) + saturation_factor * input
...
def adjust_hue_raw(input: torch.Tensor, hue_factor: Union[float, torch.Tensor]) -> torch.Tensor:
    ...
    # print('hue_factor', hue_factor.min(), hue_factor.max())
    # unpack the hsv values
    h, s, v = torch.chunk(input, chunks=3, dim=-3)
    # print('h', h.min(), h.max())
    ...
    # transform the hue value and appl module
    divisor: float = 2 * pi
    # print('h+hue_factor', (h+hue_factor).min(), (h+hue_factor).max())
    h_out: torch.Tensor = torch.fmod(h + hue_factor + divisor, divisor) # FIXME: changed here (+divisor to make sure >=0)
    # print('h_out', h_out.min(), h_out.max())
...
def adjust_hue(input: torch.Tensor, hue_factor: Union[float, torch.Tensor]) -> torch.Tensor:
    ...
    # print('input', input.min(), input.max())
    x_hsv: torch.Tensor = rgb_to_hsv(input)
    # print('x_hsv', x_hsv.min(), x_hsv.max())

    # perform the conversion
    x_adjusted: torch.Tensor = adjust_hue_raw(x_hsv, hue_factor)
    # print('x_adjusted', x_adjusted.min(), x_adjusted.max())
...
def adjust_contrast(input: torch.Tensor, contrast_factor: Union[float, torch.Tensor]) -> torch.Tensor:
    ...
    mean = torch.mean(rgb_to_grayscale(input), dim=(-3, -2, -1), keepdim=True)
    x_adjust: torch.Tensor = (1 - contrast_factor) * mean + contrast_factor * input
